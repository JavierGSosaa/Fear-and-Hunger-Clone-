...
void Player::DisplayInv() {
	...
	if (size == 0) {
		std::cout << name << "'s invetory is " << inventory << std::endl;
	}

	for (int i = 0; i < size; i++) {
		std::cout << name << "'s Item at slot " << i + 1 << " is " << inventory[i].name << ",there is " << inventory[i].amount << std::endl;
	};
	std::cout << "That is all of " << name << "'s items now returning...";
	return;
};
...
/*
Player& Player::operator<<(Item initem) {
	Player inPlayer;
	inPlayer.health = health;
	return inPlayer;
};
*/
